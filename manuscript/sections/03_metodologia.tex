...
\begin{itemize}
  \item Proporción de la muestra: Se determinó una división del 70\% para el conjunto
    de entrenamiento, 15\% para el conjunto de validación y 15\% para el conjunto
  ...
  \item Uso de los subconjuntos: El conjunto de entrenamiento se utilizó para
    ...
    interna. El conjunto de validación se reservó para comparar el rendimiento
    de los modelos y seleccionar la mejor configuración de hiperparámetros. El
    conjunto de prueba se mantuvo aislado hasta la evaluación final del modelo
    seleccionado.
\end{itemize}
...
se realizó sobre las versiones base, delegando la estandarización al pipeline
del modelo:
...
\begin{itemize}
  ...
  \item Modelos Basados en Árboles y Reglas:
    \begin{itemize}
      ...
      \item Support Vector Machine (SVM): Modelo de margen máximo con kernel radial,
        útil para capturar fronteras de decisión no lineales.
    \end{itemize}
...
utilidades propia (modeling\_utils\_tidymodels\_clean.R), lo que permite una ejecución sistemática y
...
\begin{enumerate}
  \item Validación Cruzada (5-fold Cross-Validation): Empleada para la estimación
    ...
    riesgo de sobreajuste (overfitting). Posteriormente, el conjunto de
    validación externo permitió seleccionar el mejor modelo.
  \item Entrenamiento Simple (sin CV): Utilizado para evaluar el ajuste directo de
    ...
    de los modelos en su estado base y comparar la eficiencia computacional
...
los modelos entrenados y las figuras reportadas. La estructura detallada de
...
DataExplorer, corrplot, Hmisc, knitr, kableExtra, plotly y htmltools. La
...
y funciones centralizadas en modeling\_utils\_tidymodels\_clean.R para entrenar,
...
En cada archivo, se ejecutan de forma paralela dos sub-escenarios: entrenamiento
...
receta de tidymodels, garantizando una comparación metodológicamente equivalente
entre escenarios.
...
\begin{itemize}
  ...
  \item Registro de la mejor configuración: Para conservar los hiperparámetros del
    modelo con mayor rendimiento sobre el conjunto de validación.
\end{itemize}
...
reportaron AUC-ROC, sensibilidad, especificidad y accuracy mediante funciones
centralizadas de evaluación.
...
Figuras~\ref{fig:roc-v1-cv}--\ref{fig:roc-v2-nocv} y permiten contrastar
